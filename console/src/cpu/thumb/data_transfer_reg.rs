...
fn execute(cpu: &mut CPU, memory: &mut Memory, (lbh, ro, rb, rd): (u32, u32, u32, u32)) {
    ...
    let size = match lbh {
        ...
        0b010 => {
            memory.store8(address, cpu.r[rd as usize] as u8);
            0
        }
        0b011 => {
            cpu.r[rd as usize] = memory.load8(address) as i8 as i32 as u32;
            0
        }
        0b100 => {
        ...
        0b110 => {
            cpu.r[rd as usize] = memory.load8(address) as u32;
            0
        }
        0b111 => {
            cpu.r[rd as usize] = CPU::ldrsh(address, memory);
            1
        }
        _ => unreachable!(),
    };
