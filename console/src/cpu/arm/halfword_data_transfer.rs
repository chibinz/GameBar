...
pub fn execute(
    ...
    match lsh {
        ...
        0b101 => cpu.r[rd as usize] = CPU::ldrh(address, memory) + if rd == 15 { 4 } else { 0 },
        0b110 => {
            cpu.r[rd as usize] =
                memory.load8(address) as i8 as i32 as u32 + if rd == 15 { 4 } else { 0 }
        }
        0b111 => cpu.r[rd as usize] = CPU::ldrsh(address, memory) + if rd == 15 { 4 } else { 0 },
        _ => unreachable!(),
    }
